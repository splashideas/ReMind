...
CREATE TABLE dbo.DataPoints (
    ...
    Visibility      TINYINT NOT NULL,  -- 0=Public, 1=FollowersOnly, 2=Private; no DB default — API DTO/deserializer must also require presence on create
...
JWT validation alone is not enough: register authorization with a **fallback authenticated policy** so endpoints are not anonymously callable by default, and call `app.UseAuthentication();` plus `app.UseAuthorization();` before mapping endpoints/controllers so the fallback policy is enforced. Explicitly mark only the documented exceptions with `AllowAnonymous` (today: `GET /api/erasure-requests/{id}` and `GET /healthz`)—anonymous is never the default. `GET /healthz` must stay non-sensitive (readiness/liveness only, no user data) so Azure Front Door origin probes can call it without an Entra token while origin network restrictions still limit who can reach the app. Apply explicit endpoint policies for visibility decisions (`CanViewFollowersOnly` / `CanViewPrivate`, backed by resource-based authorization handlers that evaluate the caller against the target author/chain) and a separate admin-role policy for moderation routes (`CanModerate`, which is intentionally admin-only in this design). Map the stable caller identity (`iss` + `oid`, or another explicitly linked provider key when `oid` is unavailable) to `Users.UserId` inside the API boundary; do not trust client-supplied user IDs.
...
POST   /api/users/me/avatar             # Caller only: init/replace avatar upload; requires client idempotency key + requestedSizeBytes (photo types only); creates/refreshes a row in dbo.AvatarUploads (not a Users.AvatarUploadId pointer) under avatars/{userId}/... with ClientRequestId, Status, ReservedBytes = photo per-type max, StagingBlobPath, CanonicalBlobPath, UploadExpiresUtc, and RowVersion; atomically debits Users.PendingReservedBytes by that max; concurrent replace cancels/expires any prior non-terminal reservation for the owner (releasing its counter bytes) before debiting the new row so two charged reservations cannot hide behind one pointer; returns UploadId/SAS/ReservedBytes (same reservation on key retry); optional API-proxied upload path may replace direct SAS when a hard body-size limit is required
...
GET    /healthz                         # **AllowAnonymous** readiness/liveness probe for Azure Front Door origin groups only; returns no user/domain data and is the configured health-probe path
...
Client → API (request upload URL with requestedSizeBytes + media type)
       ...
       → Queue worker reads that persisted malware verdict and obtains a clean result before any ImageSharp/FFmpeg parse; malicious results reject the upload, while failed/timed-out scans keep the object non-Ready in `Quarantined` without parsing or serving it until a clean rescan or an operator reject via `POST /api/moderation/media/{mediaId}/reject`
...
- **CDN**: Azure Front Door Premium (`azurerm_cdn_frontdoor_*`) for media delivery; the API issues short-lived, visibility-checked read URLs whose query string carries a blob-read SAS for the single canonical object, and Front Door forwards that full SAS query string to Blob Storage over the selected origin route. **Do not cache visibility-protected/SAS media routes in Front Door**: a fixed cache TTL cannot track remaining SAS lifetime, so protected reads must always revalidate at the origin. Only future public-media routes may enable caching, and then the cache key must still vary on the full authorization query string
...
4. **Visibility**
   - Explicit control: Public / Followers-only / Private (maps to `Visibility` tinyint; required on create via request-binding validation/required JSON-member handling, with no database default as defense in depth, so omission is rejected instead of silently publishing as Public). Followers-only is gated by **Accepted** follows only—open/pending follow requests do not unlock those posts.
5. **Tags**
...
| Signed media/avatar read URL TTL | ≤ 15 minutes | Front Door caching disabled for visibility-protected/SAS media routes |
...
| `azurerm_eventgrid_system_topic` + event subscription (or equivalent) | Deliver Blob-created / malware-scan result events to the cleanup and scan-result workers |
...
| `azurerm_cdn_frontdoor_profile` + endpoint/origin-group/origin/route (Premium tier) | Front Door entry for both media delivery and the public `ReMind.Api` hostname, with the API origin group probing anonymous `GET /healthz` and SAS-protected media routes configured without caching |
...
- Upload pipeline with quota counter, proxy/inspector, Defender/Event Grid scan-result flow (or equivalent), outbox, thumbnails, non-cached SAS reads
