...
CREATE TABLE dbo.AvatarUploads (
    ...
    CanonicalETag         NVARCHAR(200) NULL, -- sealed canonical blob ETag/version; set at seal; malware verdicts and Ready bind only to this value
    ...
    CONSTRAINT CK_AvatarUploads_RejectedUtc_Matches_Status CHECK (
...
1. Load `SubjectPseudonymKey` material from Key Vault for the **current** version **plus every distinct `KeyVersion` still referenced by at least one `ErasedSubjectHashes` row** (not merely versions inside a rotation retention window—once a referenced version ages out of a window the API would stop hashing with it and an erased identity could reprovision). For each such version `v`, compute `subjectHash[v] = HMAC-SHA-256(SubjectPseudonymKey[v], UTF8(Issuer) || 0x00 || UTF8(ExternalSubject))`.
2. If `ErasedSubjectHashes` contains any `subjectHash[v]` (match on `SubjectHash`, constrained by that row's `KeyVersion` when present), **do not** create a profile—return 403 `account-closed` (stable code). Erasure must not be reversible by signing in again with the same IdP subject. Do **not** use plain `SHA256(Issuer || 0x00 || ExternalSubject)`—provider subjects can be enumerable, so an unkeyed digest is only pseudonymous and is offline-attackable after a database leak.
...
POST   /api/datapoints/{id}/media/complete # Validate the existing PendingUpload row/blob against requestedSizeBytes/ReservedBytes/per-type max and seal CanonicalETag on success; failed validation transitions it to Quarantined/Rejected (set RejectedUtc when Rejected). Immediate PendingReservedBytes release is allowed only for size-enforcing proxy uploads or when write capability is genuinely revoked/expired; every direct-SAS rejection keeps the counter charged until UploadExpiresUtc/SAS se. Successful validation in one SQL transaction moves the row to Processing/PendingModeration as appropriate, converts/releases the reserved counter bytes only under that same proxy-or-expired rule (otherwise keep charged until se), writes one deduplicated OutboxMessages row, and lets the retrying dispatcher publish queue work
...
Client → API (request upload URL with requestedSizeBytes + media type)
       ...
       → Client confirms upload → API validates the staging blob exists and matches expected ETag/size/hash/signature and does not exceed `requestedSizeBytes` / `ReservedBytes` / per-type max,
        ...
        workers and read URLs use, **persists `CanonicalETag` on the media/`AvatarUploads` row**, and in one SQL transaction updates the existing media row to Quarantined (manual-hold only with `QuarantineReviewByUtc`)
        or Processing/PendingModeration/Rejected as appropriate. **Counter release rule**: immediate release/convert of `Users.PendingReservedBytes` is allowed only for size-enforcing proxy uploads (server-controlled body limit / no lingering client write SAS) or when the write SAS is already expired/revoked; every direct-SAS failed completion or rejection keeps the reservation charged until `UploadExpiresUtc` / SAS `se`,
       ...
       → Microsoft Defender for Storage malware scanning (or an equivalent required scanner) publishes result events; a scan-result worker correlates them by blob path/`UploadId` **and the sealed `CanonicalETag`**, records `MalwareScanVerdict` + `MalwareScannedUtc` idempotently on `Media`/`AvatarUploads` only when the event matches that recorded ETag/version, retries transient delivery/lookup failures, and marks `Error/Timeout` after the bounded timeout window when no clean verdict arrives
       → Expired `PendingUpload` rows/blobs (decrementing `Users.PendingReservedBytes` only once write SAS/`UploadExpiresUtc` has elapsed), expired `Quarantined` manual holds (forced to `Rejected` + `RejectedUtc`, not hard-deleted), and `Rejected` artifacts older than §8 retention from `RejectedUtc` are removed by a janitor that deletes both blob objects and media/`AvatarUploads` metadata. Direct-SAS rows that were rejected early still release their counter only at this SAS-expiry boundary
       → Queue/thumbnail workers must read that persisted malware verdict for the sealed `CanonicalETag` and obtain a clean result before any ImageSharp/FFmpeg parse; malicious results reject the upload (set `RejectedUtc`), while missing/failed/timed-out scans keep the object non-Ready in `Quarantined` without parsing or serving it until a clean rescan or an operator reject via `POST /api/moderation/media/{mediaId}/reject`
       ...
       → Media.Status becomes Ready only in a separate idempotent finalization transaction after validation + clean malware verdict + thumbnail success + moderation approval are all recorded **for the sealed `CanonicalETag`**; until then it remains non-public
...
| Rejected media retention | 30 days from `RejectedUtc` | audit/review then hard-delete blobs + row for both `Media` and `AvatarUploads`; `RejectedUtc` is required whenever `Status = Rejected` (operator reject, malware reject, inspector reject, committed-oversize reject, or quarantine-hold expiry) |
